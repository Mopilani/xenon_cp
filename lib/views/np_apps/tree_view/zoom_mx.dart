...
class ZoomMX extends StatefulWidget {
  ZoomMX({
  ...
  static Vector3 getNearestPointOnLine(Vector3 point, Vector3 l1, Vector3 l2) {
    ...
    // print('lengthSquared: $lengthSquared');
  ...
  static Vector3 getNearestPointInside(Vector3 point, Quad quad) {
    ...
    final List<Vector3> closestPoints = <Vector3>[
      ZoomMX.getNearestPointOnLine(point, quad.point0, quad.point1),
      ZoomMX.getNearestPointOnLine(point, quad.point1, quad.point2),
      ZoomMX.getNearestPointOnLine(point, quad.point2, quad.point3),
      ZoomMX.getNearestPointOnLine(point, quad.point3, quad.point0),
    ];
  ...
  @override
  State<ZoomMX> createState() => ZoomMXState();
}

class ZoomMXState extends State<ZoomMX> with TickerProviderStateMixin {
  ...
  void xonScaleUpdate(ScaleUpdateDetails details) {
  ...
  @override
  void didUpdateWidget(ZoomMX oldWidget) {
  ...
  Widget build(BuildContext context) {
    // print(
    //     '_transformationController!.value.storage.length: ${_transformationController!.value.storage.length}');
    // return widget.child;
    Widget child;
    child = _ZoomBuilt(
      childKey: _childKey,
              ...
              children: [
                ...
                ValueListenableBuilder<_ScrollBarData>(
                    ...
                    builder: (_, scrollData, __) {
                      // print(scrollData.position);
                      // print(scrollData.length);
            ...
            update: (context, state) {
              if (controlPressed) {
                return GestureDetector(
                  ...
                  // onPanDown: (v) {
                  //   if (controlePress) {
                  //     onSellectionStarted(TapDownDetails(
                  //         localPosition :
                  //             Offset(v.localPosition.dx, v.localPosition.dy)));
                  //     return;
                  //   } else {}
                  // },
...
Quad _getAxisAlignedBoundingBoxWithRotation(Rect rect, double rotation) {
  ...
  return ZoomMX.getAxisAlignedBoundingBox(boundariesRotated);
}
...
Offset _exceedsBy(Quad boundary, Quad viewport) {
  ...
  for (final Vector3 point in viewportPoints) {
    final Vector3 pointInside = ZoomMX.getNearestPointInside(point, boundary);
